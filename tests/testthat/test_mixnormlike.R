...
test_that("estimate_mixprop works with errordist set") {
    set.seed(889)
    k1 <- 5
    k2 <- 7
    a_seq1 <- seq(-5, 0, length = k1)
    ...
    prior <- rep(1, length = k1)

    sebetahat <- rep(NA, length = p)


    mix_out <- estimate_mixprop(betahat = betahat, sebetahat = sebetahat,
                                g = g, prior = prior,
                                errordist = errordist)
}
